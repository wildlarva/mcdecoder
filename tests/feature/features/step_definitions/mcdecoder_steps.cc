...
BEFORE_ALL() { mcdhelper::SetupDecoders(); }
...
WHEN("^I decode \"([^\"]+)\"$") {
  ...
  constexpr int kByteCharLen = 2;
  int byte_length = std::min(static_cast<int>(code_str.length() / kByteCharLen), 4);

  // Convert hex string to integer array
  uint8_t codes[4];
  for (int i = 0; i < byte_length; i++) {
    codes[i] = std::stoul(code_str.substr(i * kByteCharLen, kByteCharLen), /*pos=*/nullptr, 16);
  }

  // Decode
  mcdhelper::DecodeRequest request;
  request.decoder_name = context->decoder_name;
  request.codes = reinterpret_cast<const uint8_t*>(&codes[0]);
...
THEN("^the fields \"([^\"]+)\" should be \"([^\"]+)\"$") {
  ...
  for (int i = 0; i < fields.size(); i++) {
    ...
    uint32_t expected_value = std::stoul(str_expected_value, /*pos=*/nullptr, 16);
